...
async function main() {
  ...
  try {
    ...
    await fs.cp(path.join(repoRoot, "src", "node", "cli"), path.join(distDir, "src", "node", "cli"), { recursive: true });
    await fs.cp(path.join(repoRoot, "src", "node", "shared"), path.join(distDir, "src", "node", "shared"), { recursive: true });
...
(async () => {
  const cliPath = path.join(path.dirname(process.execPath), 'src', 'node', 'cli', 'cli.js');
  const module = await import(pathToFileURL(cliPath).href);

  if (typeof module.main !== 'function') {
    throw new Error('The CLI entrypoint did not export main().');
  }
