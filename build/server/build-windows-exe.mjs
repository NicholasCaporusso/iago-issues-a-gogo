...
async function main() {
  ...
  try {
    ...
    await fs.cp(path.join(repoRoot, "src", "server"), path.join(distDir, "src", "server"), {
      errorOnExist: false,
      force: false,
      recursive: true
    });
    await fs.cp(path.join(repoRoot, "src", "shared"), path.join(distDir, "src", "shared"), {
      errorOnExist: false,
      force: false,
      recursive: true
    });
...
(async () => {
  const serverPath = path.join(path.dirname(process.execPath), 'src', 'server', 'server.js');
