...
async function main() {
  ...
  try {
    ...
    await fs.cp(path.join(repoRoot, "src", "cli"), path.join(distDir, "src", "cli"), { recursive: true });
    await fs.cp(path.join(repoRoot, "src", "shared"), path.join(distDir, "src", "shared"), { recursive: true });
...
(async () => {
  const cliPath = path.join(path.dirname(process.execPath), 'src', 'cli', 'cli.js');
