...
async function commitIssueFix({
  ...
  branch = null,
  runner = runGitCommand,
  issueCloser = shared.closeRemoteIssue
}) {
  if (!issueNumber && issueNumber !== 0) {
  ...
  const commitMessage = shared.buildIssueFixCommitMessage(issueNumber, { title, description });
...
async function relayCompleted(repoRoot, options) {
  const remoteName = options.remote ?? "origin";
  const { remoteUrl } = await shared.resolveRepositoryContext(repoRoot, remoteName);
  ...
  const response = await fetch(relayTarget, {
    ...
    body: JSON.stringify({
      ...
      description: options.description,
        ...
        repositoryUrl: shared.normalizeRepositoryRemote(remoteUrl),
        save: options.save,
        title: options.title
      })
  });
  ...
  return {
    ...
    commitMessage: body.commitMessage ?? shared.buildIssueFixCommitMessage(options.issueNumber, options),
